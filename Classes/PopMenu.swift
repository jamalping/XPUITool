...
//  TestSwift
//
//  Created by lyj on 18/5/7.
//  Copyright © 2018年 zyyj. All rights reserved.
//

import UIKit

@objc public protocol SwiftPopMenuDelegate {
    func swiftPopMenuDidSelectIndex(pop:PopMenu,index:Int)
}

// MARK: 弹出选择菜单
public class PopMenu: UIView {
    
    public weak var delegate : SwiftPopMenuDelegate?
    ...
    private var widthValue : CGFloat = 100
    //／*  -----------------------  可变参数 ------------------------------------------ *／
    ...
    //pop文字颜色
    public var popTextColor:UIColor = UIColor(red: 242 / 255.0, green: 88 / 255.0, blue: 79 / 255.0, alpha: 1.0)
    //pop背景色
    ...
    public var popData: [(icon: String?,title: String)] = [(icon: String?,title: String)](){
        didSet{
            ...
            self.myFrame = CGRect.init(origin: touchPoint!, size: CGSize.init(width: self.myFrame.width, height: CGFloat(popData.count)*self.rowHeightValue+arrowViewMargin))
     ...
     */
    init(width: CGFloat = 100, rowHeight: CGFloat = 44, touchPoint: CGPoint) {
        ...
        self.myFrame = CGRect.init(origin: touchPoint, size: CGSize.init(width: width, height: CGFloat(popData.count)*self.rowHeightValue+arrowViewMargin))
...
//    }
    ...
    func initViews() {
        ...
        //箭头
        arrowView=UIView(frame: CGRect(x: myFrame.origin.x - myFrame.width + arrowViewWidth/2 + arrowViewMargin, y: myFrame.origin.y, width: myFrame.width, height: arrowViewHeight))
        ...
        tableView=UITableView(frame: CGRect(x: myFrame.origin.x - myFrame.width + arrowViewWidth/2 + arrowViewMargin,y: myFrame.origin.y + arrowViewHeight,width: myFrame.width,height: myFrame.height - arrowViewHeight), style: .plain)
        tableView.register(SwiftPopMenuCell.classForCoder(), forCellReuseIdentifier: PopMenu.cellID)
        ...
        /// 超出屏幕范围时，调整位置、
        let winPoint = self.convert(CGPoint.init(x: self.tableView.frame.width + self.tableView.frame.origin.x, y: self.tableView.frame.height + self.tableView.frame.origin.y), to: keyWindow)
        
        if winPoint.x > KScrW {
            print("w")
            self.tableView.right = KScrW
            //            self.tableView.frame.origin.x = KScrW - myFrame.width
        }
        
        if winPoint.x - self.tableView.width < 0 {
        ...
        if winPoint.y > KScrH {
            ...
            self.tableView.bottom = self.touchPoint!.y
        }
...
class SwiftPopMenuCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        //        lblTitle.textColor = UIColor(red: 242/255.0, green: 88/255.0, blue: 79/255.0, alpha: 1)
    ...
    func fill(iconImage: String?,title:String,textColor:UIColor,islast:Bool = false) {
        if let iImage = iconImage {
            self.iconImage.image = UIImage(named: iImage)
        }
...
extension PopMenu : UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if popData.count>indexPath.row {
            let cell = tableView.dequeueReusableCell(withIdentifier: PopMenu.cellID) as! SwiftPopMenuCell
...
extension PopMenu : UITableViewDelegate{
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.delegate != nil{
            self.delegate?.swiftPopMenuDidSelectIndex(pop: self, index: indexPath.row)
        }
