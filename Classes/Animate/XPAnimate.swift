...
import UIKit
/// 基础动画
class XPAnimationGroup: NSObject {
    
    /// 动画开始回调
    var startAnimation: ((CAAnimation) -> ())?
    /// 动画结束回调
    var stopAnimation: ((CAAnimation, Bool) -> ())?
    
    func animationGroup(duration: CFTimeInterval, animations: [CAAnimation]) -> CAAnimationGroup {
...
extension XPAnimationGroup: CAAnimationDelegate {
